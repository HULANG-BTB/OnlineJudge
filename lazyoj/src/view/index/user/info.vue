  ...
  <div :style="{marginTop: '3.5rem'}">
    <Row>
      <Col :offset="8" :span="8">
        <Card>
          <div slot="title">
            <h1 :style="{textAlign: 'center'}">信息修改</h1>
          </div>
          <Form
            ref="table"
            :model="formData"
          ...
          >
            <FormItem label="用户名" prop="username">
              <Input v-model="formData.username" placeholder="请输入用户名" readonly disabled></Input>
            </FormItem>
...
export default {
  ...
  methods: {
    handleSubmitClick() {
      this.loading = true;
      this.$axios
        .post("/api/user/update", this.formData)
        .then(res => {
          if (res.data.code === 1) {
  ...
  watch: {
    UserInfo() {
      for (let i in this.formData) {
        this.formData[i] = this.$store.state.User[i];
      }
    }
  },
  mounted() {
