...
void init_mysql_conf() {
	...
	if (fp != NULL) {
		while (fgets(buf, BUFFER_SIZE - 1, fp)) {
			...
			read_buf(buf, "OJ_DATABASE_HOST", host_name);
			read_buf(buf, "OJ_DATABASE_USER", user_name);
			read_buf(buf, "OJ_DATABASE_PASS", password);
			read_buf(buf, "OJ_DATABASE_NAME", db_name);
			read_int(buf, "OJ_DATABASE_PORT", &port_number);
			read_int(buf, "OJ_MAX_RUNNING", &max_running);
			read_int(buf, "OJ_SLEEP_TIME", &sleep_time);
			read_int(buf, "OJ_TOTAL", &oj_tot);

			read_buf(buf, "OJ_HOME_PATH", oj_home);

			read_int(buf, "OJ_MOD", &oj_mod);

			read_int(buf, "OJ_HTTP_JUDGE", &http_judge);
			read_buf(buf, "OJ_HTTP_BASEURL", http_baseurl);
			read_buf(buf, "OJ_HTTP_KEY", http_username);
			...
			read_int(buf, "OJ_REDIS_ENABLE", &oj_redis);
			read_buf(buf, "OJ_REDIS_SERVER", oj_redisserver);
			read_int(buf, "OJ_REDIS_PORT", &oj_redisport);
			read_buf(buf, "OJ_REDIS_AUTH", oj_redisauth);
			read_buf(buf, "OJ_REDIS_QNAME", oj_redisqname);
...
#ifdef _mysql_h
		sprintf(query,
				"SELECT sid FROM solution WHERE language in (%s) and result < 2 and MOD(sid,%d)=%d ORDER BY result ASC,sid ASC limit %d",
...
bool _check_out_mysql(int solution_id, int result) {
	...
	sprintf(sql,
			"UPDATE solution SET result=%d,time=0,memory=0,updatedAt=NOW() WHERE sid=%d and result<2 LIMIT 1",
			result, solution_id);
	if (mysql_real_query(conn, sql, strlen(sql))) {
