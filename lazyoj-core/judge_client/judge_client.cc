...
/*
 * File: judge_client.cc
 * Revised on 2020 for undergraduate graduation design
 */
...
// OJ 数据库相关配置 文件读取
static char	OJ_DATABASE_HOST[BUFFER_SIZE] = "localhost"; // 数据库地址
static char	OJ_DATABASE_USER[BUFFER_SIZE] = "root"; // 数据库用户名
static char	OJ_DATABASE_PASS[BUFFER_SIZE] = "root"; // 数据库密码
static char	OJ_DATABASE_NAME[BUFFER_SIZE] = "jol"; // 数据库名
static int	OJ_DATABASE_PORT = 3306; // 数据库端口
// OJ 配置 文件读取
static double	OJ_CPU_COMPENSATION	= 1.0; // CPU 补偿 CPU速度较快 则调大 反之调小
static int	OJ_MAX_RUNNING = 3; // 最大同时运行数目
static int	OJ_SLEEP_TIME = 3; // 默认休眠时间
static int	OJ_JAVA_TIME_BONUS		= 5; // JAVA程序需要的额外运行时间
static int	OJ_JAVA_MEMORY_BONUS	= 512; // JAVA程序需要的额外运行内存
static char	OJ_JAVA_XMS[BUFFER_SIZE]; // JAVA程序最大运行内存
static char	OJ_JAVA_XMX[BUFFER_SIZE]; // JAVA程序初始化时的内存
static int	OJ_SIM_ENABLE		= 0; // OJ是否启用相似度检测
static int	OJ_JUDGE_MODE	= 0; // OJ评测模式 1 ACM 2 OI 从数据库读取
static int	OJ_FULL_DIFF		= 0;	// 是否完全匹配结果
static int	OJ_USE_MAX_TIME		= 0; // 是否启用数据中最大运行时间为最终运行时间
static int	OJ_TIME_LIMIT_TO_TOTAL	= 0; // 是否限制总的运行时间
static int	OJ_SHM_RUM = 0; // 是否使用/dev/shm的共享内存虚拟磁盘来运行答案，如果启用能提高判题速度，但需要较多内存
static int	OJ_USE_PTRACE	= 1; // 使用ptrace方式调试程序
static int	OJ_COMPILE_CHROOR	= 1; // 使用chroot环境测试程序 保障权限安全
static int	OJ_TURBO_MODE	= 0; // 是否忽略部分中间结果 以提高评测效率

// OJ默认目录 读取数据文件相关
static char	OJ_HOME_PATH[BUFFER_SIZE]; // OJ所在目录
static char	OJ_JUDGER_NAME[BUFFER_SIZE]; // 评测机名称

// 数据列表
static char	DATA_LIST[BUFFER_SIZE][BUFFER_SIZE]; // 数据文件列表
static int	DATA_LIST_LEN = 0; // 数据文件数目

// OJ 变量
static int	total_time		= 0; // 程序运行总时间
static char	record_call	= 0; // 启用 call 记录 用于 debug
static const char *tbname = "solution";  // 数据库表名称
static int py2 = 1; // caution: py2=1 means default using py3 默认使用py3

// HTTP 方式评测 配置
static int	OJ_HTTP_JUDGE = 0; // 是否启用HTTP
static char	OJ_HTTP_BASEURL[BUFFER_SIZE]; // 基础地址
static char	OJ_HTTP_KEY[BUFFER_SIZE]; // 验证key

// 部分即将舍弃的配置
static char	http_OJ_DATABASE_PASS[BUFFER_SIZE];
...
void write_log( const char *_fmt, ... )
{
	 ...
	 * int l;
	 */
	sprintf( buffer, "%s/log/client.log", OJ_HOME_PATH );
...
// 去除字符串首尾空格 如字符串存在多个空格，返回第一个非头空格前字符串
void trim( char *c )
{
...
// 读取buff为key => value // 用于读取CONF文件 读取为char*类型
...
// 读取buff为key => value // 用于读取CONF文件 读取为double类型
...
// 读取buff为key => value // 用于读取CONF文件 读取为int类型
...
void init_mysql_conf()
{
	FILE	*fp = NULL;
	char	buf[BUFFER_SIZE];
	strcpy( OJ_JAVA_XMS, "-Xms32m" );
	strcpy( OJ_JAVA_XMX, "-Xmx256m" );
	sprintf( buf, "%s/etc/judge.conf", OJ_HOME_PATH ); // 自定义OJ目录 
	fp = fopen( buf, "re" ); // 配置文件地址
	if ( fp != NULL )
	{
		while ( fgets( buf, BUFFER_SIZE - 1, fp ) )
		{
			...
			// 数据库配置
			read_buf( buf, "OJ_DATABASE_HOST", OJ_DATABASE_HOST ); // 数据库地址
			read_buf( buf, "OJ_DATABASE_USER", OJ_DATABASE_USER ); // 数据库账户
			read_buf( buf, "OJ_DATABASE_PASS", OJ_DATABASE_PASS ); // 数据库密码
			read_buf( buf, "OJ_DATABASE_NAME", OJ_DATABASE_NAME ); // 数据库名称
			read_int( buf, "OJ_DATABASE_PORT", &OJ_DATABASE_PORT ); // 数据库端口

			// OJ 配置
			read_double( buf, "OJ_CPU_COMPENSATION", &OJ_CPU_COMPENSATION );
			read_int( buf, "OJ_MAX_RUNNING", &OJ_MAX_RUNNING );
			read_int( buf, "OJ_SLEEP_TIME", &OJ_SLEEP_TIME );
			read_int( buf, "OJ_JAVA_TIME_BONUS", &OJ_JAVA_TIME_BONUS );
			read_int( buf, "OJ_JAVA_MEMORY_BONUS", &OJ_JAVA_MEMORY_BONUS );
			read_buf( buf, "OJ_JAVA_XMS", OJ_JAVA_XMS );
			read_buf( buf, "OJ_JAVA_XMX", OJ_JAVA_XMX );
			// read_int( buf, "OJ_SIM_ENABLE", &OJ_SIM_ENABLE );
			read_int( buf, "OJ_FULL_DIFF", &OJ_FULL_DIFF );
			read_int( buf, "OJ_USE_MAX_TIME", &OJ_USE_MAX_TIME );
			read_int( buf, "OJ_TIME_LIMIT_TO_TOTAL", &OJ_TIME_LIMIT_TO_TOTAL );
			read_int( buf, "OJ_SHM_RUM", &OJ_SHM_RUM );
			read_int( buf, "OJ_USE_PTRACE", &OJ_USE_PTRACE );
			read_int( buf, "OJ_COMPILE_CHROOR", &OJ_COMPILE_CHROOR );
			read_int( buf, "OJ_TURBO_MODE", &OJ_TURBO_MODE );

			// HTTP 评测配置
			read_int( buf, "OJ_HTTP_JUDGE", &OJ_HTTP_JUDGE );
			read_buf( buf, "OJ_HTTP_BASEURL", OJ_HTTP_BASEURL );
			read_buf( buf, "OJ_HTTP_KEY", OJ_HTTP_KEY );
			read_buf( buf, "OJ_JUDGER_NAME", OJ_JUDGER_NAME );
			
			// read_int( buf, "OJ_JUDGE_MODE", &OJ_JUDGE_MODE );
		}
		if ( fp != NULL ) {
			fclose(fp);
		}
...
// 文件对比方式 原 hustoj 消耗内存较大 且有文件限制
int compare_hustoj(const char *file1, const char *file2) {
	FILE	*f1, *f2;
	char	*s1, *s2, *p1, *p2;
	int	PEflg;
...
int compare( const char *file1, const char *file2 )
{
	 ...
	 * and waste memory
	 */
	return compare_hustoj( file1, file2 );
#endif
}
...
bool check_login()
{
	...
	int	ret	= 0;
	FILE	*fjobs	= read_cmd_output( cmd, OJ_HTTP_BASEURL );
	fscanf( fjobs, "%d", &ret );
	pclose( fjobs );
...
void login()
{
	...
	{
		const char *cmd =
			"wget --post-data=\"user_id=%s&OJ_DATABASE_PASS=%s\" --load-cookies=cookie --save-cookies=cookie --keep-session-cookies -q -O - \"%s/login.php\"";
		FILE *fjobs = read_cmd_output( cmd, OJ_JUDGER_NAME, http_OJ_DATABASE_PASS,
					       OJ_HTTP_BASEURL );
		/* fscanf(fjobs,"%d",&ret); */
		pclose( fjobs );
	}
...
#ifdef _mysql_h
// 回写评测结果到数据库表中 mysql 方式
void _update_solution_mysql( int sid, int result, int time, int memory, int sim, int sim_s_id, double pass_rate )
{
	char	sql[BUFFER_SIZE];
	char	judger[BUFFER_SIZE];
	mysql_real_escape_string( conn, judger, OJ_JUDGER_NAME, strlen( OJ_JUDGER_NAME ) );

	if ( OJ_JUDGE_MODE )
	{
		sprintf( sql,
			 "UPDATE %s SET result=%d,time=%d,memory=%d,pass_rate=%f,judger='%s',updatedAt=now() WHERE sid=%d ",
			 tbname, result, time, memory, pass_rate, judger, sid );
	} else {
		sprintf( sql,
			 "UPDATE %s SET result=%d,time=%d,memory=%d,judger='%s',updatedAt=now() WHERE sid=%d ",
			 tbname, result, time, memory, judger, sid );
	}
	/*      printf("sql= %s\n",sql); */
	printf("sql:[%s]\n", sql);
...
void _update_solution_http( int solution_id, int result, int time, int memory, int sim, int sim_s_id, double pass_rate )
{
	...
	FILE *fjobs = read_cmd_output( cmd, solution_id, result, time, memory, sim,
				       sim_s_id, pass_rate, OJ_HTTP_BASEURL );
	/* fscanf(fjobs,"%d",&ret); */
	pclose( fjobs );
}
...
#ifdef _mysql_h
// 回写 编译信息到数据库函数 MYSQL 方式
void _addceinfo_mysql( int solution_id )
{
	char	sql[(1 << 16)], *end;
	char	ceinfo[(1 << 16)], *cend;
	FILE	*fp = fopen( "ce.txt", "re" );
	snprintf( sql, (1 << 16) - 1, "DELETE FROM compile_info WHERE sid=%d", solution_id );
	...
	*cend = '\0';
	...
	strcpy( end, "INSERT INTO compile_info VALUES(" );
	end	+= strlen( sql );
	*end++	= '\'';
...
void _addreinfo_mysql( int solution_id, const char *filename )
{
	...
	*rend	= '\0';
	...
	strcpy( end, "INSERT INTO runtime_info VALUES(" );
	end	+= strlen( sql );
	*end++	= '\'';
	end	+= sprintf( end, "%d", solution_id );
	*end++	= '\'';
...
#ifdef _mysql_h
// 更新用户统计 AC 和 SUBMIT 数目 MYSQL 方式 拟交由 触发器完成
void _update_user_mysql( char *user_id )
{
	char sql[BUFFER_SIZE];
	sprintf( sql,
		 "UPDATE `user` SET `accepted`=(SELECT count(DISTINCT `pid`) FROM `solution` WHERE `uid`=\'%s\' AND `result`=4) WHERE `uid`=\'%s\'",
		 user_id, user_id );
	if ( mysql_real_query( conn, sql, strlen( sql ) ) )
		write_log( mysql_error( conn ) );
	sprintf( sql,
		 "UPDATE `user` SET `submit`=(SELECT count(*) FROM `solution` WHERE `uid`=\'%s\' and pid > 0) WHERE `uid`=\'%s\'",
		 user_id, user_id );
	if ( mysql_real_query( conn, sql, strlen( sql ) ) )
		write_log( mysql_error( conn ) );
}
...
// 更新用户统计 AC 和 SUBMIT 数目 HTTP 方式 拟交由 触发器完成
void _update_user_http( char *user_id )
{
	...
	FILE *fjobs = read_cmd_output( cmd, user_id, OJ_HTTP_BASEURL );
	/* fscanf(fjobs,"%d",&ret); */
	pclose( fjobs );
}

// 更新用户统计 AC 和 SUBMIT 数目 （同步数据）
...
#ifdef _mysql_h
// 更新题目统计 AC 和 SUBMIT 数目 MYSQL 方式 拟交由 触发器完成
void _update_problem_mysql( int p_id, int cid )
{
	...
	{
		sprintf( sql,
			 "UPDATE `contest_problem` SET `c_accepted`=(SELECT count(*) FROM `solution` WHERE `pid`=%d AND `result`=4 and cid=%d) WHERE `pid`=%d and cid=%d",
			 p_id, cid, p_id, cid );
		printf( "sql:[%s]\n", sql );
		if ( mysql_real_query( conn, sql, strlen( sql ) ) ) {
			write_log( mysql_error( conn ) );
	...
	sprintf( sql,
		 "UPDATE `problem` SET `accepted`=(SELECT count(*) FROM `solution` WHERE `pid`=%d AND `result`=4) WHERE `pid`=%d",
		 p_id, p_id );
	printf( "sql:[%s]\n", sql );
	if ( mysql_real_query( conn, sql, strlen( sql ) ) )
		write_log( mysql_error( conn ) );
	...
	{
		sprintf( sql,
			 "UPDATE `contest_problem` SET `c_submit`=(SELECT count(*) FROM `solution` WHERE `pid`=%d AND cid=%d) WHERE `pid`=%d and cid=%d",
			 p_id, cid, p_id, cid );
		if ( mysql_real_query( conn, sql, strlen( sql ) ) )
			write_log( mysql_error( conn ) );
	}
	...
	sprintf(sql,
	"UPDATE `problem` SET `submit`=(SELECT count(*) FROM `solution` WHERE `pid`=%d) WHERE `pid`=%d",
	p_id, p_id);
	printf( "sql:[%s]\n", sql );
	if (mysql_real_query(conn, sql, strlen(sql))) {
		write_log(mysql_error(conn));
...
// 更新题目统计 AC 和 SUBMIT 数目 HTTP 方式 拟交由 触发器完成
void _update_problem_http( int pid, int cid )
{
	...
	FILE * fjobs = read_cmd_output( cmd, pid, OJ_HTTP_BASEURL );
	/* fscanf(fjobs,"%d",&ret); */
	pclose( fjobs );
}

// 更新题目统计 AC 和 SUBMIT 数目 入口 （数据同步）
...
int compile( int lang, char *work_dir )
{
	...
	sprintf( CP_J[1], "-J%s", OJ_JAVA_XMS );
	sprintf( CP_J[2], "-J%s", OJ_JAVA_XMX );
	...
	if ( pid == 0 )
	{
		...
		if ( OJ_COMPILE_CHROOR && lang != 3 && lang != 9 && lang != 6 && lang != 11 )
		{
...
int init_mysql_conn()
{
	...
	/* mysql_real_connect(conn,OJ_DATABASE_HOST,OJ_DATABASE_USER,OJ_DATABASE_PASS,OJ_DATABASE_NAME,OJ_DATABASE_PORT,0,0); */
	const char timeout = 30;
	mysql_options( conn, MYSQL_OPT_CONNECT_TIMEOUT, &timeout );
	if ( !mysql_real_connect( conn, OJ_DATABASE_HOST, OJ_DATABASE_USER, OJ_DATABASE_PASS, OJ_DATABASE_NAME, OJ_DATABASE_PORT, 0, 0 ) )
	{
...
void _get_solution_mysql( int solution_id, char *work_dir, int lang )
{
	...
	MYSQL_ROW	row;
	int nums = 0;
	sprintf( sql, "SELECT source FROM source_code WHERE sid=%d",
		 solution_id );
	mysql_real_query( conn, sql, strlen( sql ) );
...
void _get_solution_http( int solution_id, char *work_dir, int lang )
{
	...
	FILE *pout = read_cmd_output( cmd2, solution_id, src_pth, OJ_HTTP_BASEURL );
...
// 读取用户提交的测试输入  未实现    begin    ---------
#ifdef _mysql_h
// 读取用户提交的测试输入 mysql 方式
...
#ifdef _mysql_h
//读取solution信息 mysql 方式
void _get_solution_info_mysql( int solution_id, int & p_id, char * user_id, int & lang, int &cid )
{
	MYSQL_RES	*res;
	MYSQL_ROW	row;
	...
	if ( OJ_TURBO_MODE == 2 )
	{
		...
		sprintf( sql,
			 "SELECT pid, uid, language,cid FROM solution where sid=%d",
			 solution_id );
	}
	/* printf("%s\n",sql); */
	mysql_real_query( conn, sql, strlen( sql ) );
	...
	// --- read judgemode from contest add by lang hu 2020-04-20 begin	-----
	if (cid != 0) {
		sprintf( sql,
			 "SELECT judgemode,sim FROM contest where cid=%d",
			 cid );
		mysql_real_query( conn, sql, strlen( sql ) );
		res	= mysql_store_result( conn );
		row	= mysql_fetch_row( res );
		OJ_JUDGE_MODE = atoi( row[0] );
		OJ_SIM_ENABLE = atoi( row[1] );
		printf("OJ_JUDGE_MODE: %s\n", OJ_JUDGE_MODE ? "True" : "False");
		printf("OJ_SIM_ENABLE: %s\n", OJ_SIM_ENABLE ? "True" : "False");
		if ( res != NULL )
		{
			mysql_free_result( res );         /* free the memory */
			res = NULL;
	...
	// --- read judgemode from contest add by lang hu 2020-04-20 end	-----
}
#endif

//读取solution信息 mysql 方式
void _get_solution_info_http( int solution_id, int & p_id, char * user_id, int & lang, int & cid )
{
	login();
	...
	FILE *pout = read_cmd_output( cmd, solution_id, OJ_HTTP_BASEURL );
	fscanf( pout, "%d", &p_id );
...
#ifdef _mysql_h
// 读取问题信息 mysql 方式
void _get_problem_info_mysql( int p_id, double &time_lmt, int &mem_lmt, int &isspj )
{
	...
	MYSQL_ROW	row;
	sprintf( sql,
		 "SELECT time_limit,memory_limit,spj FROM problem where pid=%d",
		 p_id );
	mysql_real_query( conn, sql, strlen( sql ) );
	res		= mysql_store_result( conn );
	row		= mysql_fetch_row( res );
...
#endif
// 读取问题信息 http 方式
void _get_problem_info_http( int p_id, double &time_lmt, int &mem_lmt, int &isspj )
{
	/* login(); */
	...
	FILE *pout = read_cmd_output( cmd, p_id, OJ_HTTP_BASEURL );
...
void prepare_files( char *filename, int namelen, char *infile, int &p_id, char *work_dir, char *outfile, char *userfile, int runner_id )
{
	...
	escape( fname, fname0 );
	/* printf("%s\n%s\n",fname0,fname); */
	sprintf( infile, "%s/data/%d/%s.in", OJ_HOME_PATH, p_id, fname );
	execute_cmd( "/bin/cp '%s' %s/data.in", infile, work_dir );
	execute_cmd( "/bin/cp %s/data/%d/*.dic %s/ 2>/dev/null", OJ_HOME_PATH, p_id, work_dir );

	sprintf( outfile, "%s/data/%d/%s.out", OJ_HOME_PATH, p_id, fname0 );
	sprintf( userfile, "%s/run/%d/user.out", OJ_HOME_PATH, runner_id );
}
...
// 运行解决方案
void run_solution( int &lang, char *work_dir, double &time_lmt, int &usedtime, int &mem_lmt )
{
	// 设置优先度最高
	nice( 19 );
...
/*		&& lang != 6 */
	...
	/* time limit */
	if ( OJ_TIME_LIMIT_TO_TOTAL )
		LIM.rlim_cur = (time_lmt / OJ_CPU_COMPENSATION - usedtime / 1000.0f) + 1;
	else
		LIM.rlim_cur = time_lmt / OJ_CPU_COMPENSATION + 1;
	LIM.rlim_max = LIM.rlim_cur;
	/* if(DEBUG) printf("LIM_CPU=%d",(int)(LIM.rlim_cur)); */
	setrlimit( RLIMIT_CPU, &LIM );
	alarm( 0 );
	alarm( 1 + time_lmt / OJ_CPU_COMPENSATION );
	...
	case 19:
		...
		break;
	case 3:
		sprintf( OJ_JAVA_XMX, "-Xmx%dM", mem_lmt );
		/* sprintf(OJ_JAVA_XMX, "-XX:MaxPermSize=%dM", mem_lmt); */

		execl( "/usr/bin/java", "/usr/bin/java", OJ_JAVA_XMX,
...
// 特殊判定方式 运行
int special_judge( char *OJ_HOME_PATH, int problem_id, char *infile, char *outfile, char *userfile )
{
	pid_t pid;
	printf( "pid=%d\n", problem_id );
	pid = fork();
	...
	if ( pid == 0 )
	{
		...
		ret = execute_cmd( "%s/data/%d/spj '%s' '%s' %s", OJ_HOME_PATH, problem_id,
		     ...
		     int solution_id, int num_of_test )
{
	...
	{
		int real_limit = 1000;
		if ( OJ_TIME_LIMIT_TO_TOTAL )              /* 如果限制总时间 */
		{
	...
	if ( ACflg == OJ_AC )
	{
		if ( isspj )
		{
			comp_res = special_judge( OJ_HOME_PATH, p_id, infile, outfile, userfile );
		     ...
		     int &PEflg, char *work_dir )
{
	...
	while ( 1 )
	{
		...
		if ( WIFEXITED( status ) )
			break;
		if ( (lang < 4 || lang == 9) && get_file_size( "error.out" ) && !OJ_JUDGE_MODE )
		{
			ACflg = OJ_RE;
		...
		if ( (lang >= 3 && exitcode == 17) || exitcode == 0x05 || exitcode == 0 || exitcode == 133 )
			...
			if ( ACflg == OJ_AC )
			{
				...
				print_runtimeerror( infile + strlen( OJ_HOME_PATH ) + 5, strsignal( exitcode ) );
			}
			ptrace( PTRACE_KILL, pidApp, NULL, NULL );
		...
		if ( WIFSIGNALED( status ) )
		{
			...
			if ( ACflg == OJ_AC )
			{
				...
				print_runtimeerror( infile + strlen( OJ_HOME_PATH ) + 5, strsignal( sig ) );
			}
			break;
		}
 ...
 */
	...
	else if ( call_counter[call_id] )
		{
			...
			write_log( error );
			print_runtimeerror( infile + strlen( OJ_HOME_PATH ) + 5, error );
			ptrace( PTRACE_KILL, pidApp, NULL, NULL );
		}
#ifdef __mips__
	}
/*		} */
	...
	usedtime	+= (ruse.ru_utime.tv_sec * 1000 + ruse.ru_utime.tv_usec / 1000) * OJ_CPU_COMPENSATION;
	usedtime	+= (ruse.ru_stime.tv_sec * 1000 + ruse.ru_stime.tv_usec / 1000) * OJ_CPU_COMPENSATION;
...
void clean_workdir( char *work_dir )
{
	...
	if ( DEBUG )
	{
		...
		execute_cmd( "mkdir -p %s/log/ 2>/dev/null", work_dir );
		execute_cmd( "/bin/mv %s/* %s/log/ 2>/dev/null", work_dir, work_dir );
		execute_cmd( "/bin/rm -rf %s/log/* 2>/dev/null", work_dir );
	}
}
...
void init_parameters( int argc, char **argv, int &solution_id, int &runner_id )
{
	if ( argc < 3 )
	{
		...
		fprintf( stderr, "Multi:%s solution_id runner_id judge_base_path.\n",
			 argv[0] );
		fprintf( stderr,
			 "Debug:%s solution_id runner_id judge_base_path oj_sim_enable debug.\n",
			 argv[0] );
		exit( 1 );
	}
	DEBUG		= (argc > 4);
	...
	if ( argc > 3 ) {
		strcpy( OJ_HOME_PATH, argv[3] );
	} else {
		strcpy( OJ_HOME_PATH, "/home/judge" );
	}
		
	chdir( OJ_HOME_PATH ); /* change the dir// init our work */
...
int get_sim( int solution_id, int lang, int pid, int &sim_s_id )
{
	...
	if ( !sim )
	{
		...
		execute_cmd( "/bin/cp %s ../../data/%d/ac/%d.%s 2>/dev/null", src_pth, pid, solution_id,
			     lang_ext[lang] );
		if(DEBUG)
			printf( "/bin/cp %s ../../data/%d/ac/%d.%s 2>/dev/null", src_pth, pid, solution_id,
			     lang_ext[lang] );		 
		/* c cpp will */
		if ( lang == 0 )
...
void mk_shm_workdir( char *work_dir )
{
	...
	execute_cmd( "/bin/ln -s %s %s/  2>/dev/null", shm_path, OJ_HOME_PATH );
	execute_cmd( "/bin/chown judge %s  2>/dev/null", shm_path );
	execute_cmd( "chmod 755 %s  2>/dev/null", shm_path );
	/* sim need a soft link in shm_dir to work correctly */
	sprintf( shm_path, "/dev/shm/hustoj/%s/", OJ_HOME_PATH );
	execute_cmd( "/bin/ln -s %s/data %s  2>/dev/null", OJ_HOME_PATH, shm_path );
}
...
int get_test_file( char *work_dir, int p_id )
{
	...
	FILE *fjobs = read_cmd_output( cmd, p_id, OJ_HTTP_BASEURL );
	while ( fgets( filename, BUFFER_SIZE - 1, fjobs ) != NULL )
	{
		...
		if ( OJ_HTTP_JUDGE && (!DATA_LIST_has( filename ) ) )
			DATA_LIST_add( filename );
		sprintf( localfile, "%s/data/%d/%s", OJ_HOME_PATH, p_id, filename );
		if ( DEBUG )
			printf( "localfile[%s]\n", localfile );
		...
		if ( access( localfile, 0 ) == -1 || local_date < remote_date )
		{
			if ( strcmp( filename, "spj" ) == 0 )
				continue;
			execute_cmd( "/bin/mkdir -p %s/data/%d", OJ_HOME_PATH, p_id );
			const char *cmd2 =
			...
			execute_cmd( cmd2, p_id, filename, localfile, OJ_HTTP_BASEURL );
			ret++;

			if ( strcmp( filename, "spj.c" ) == 0 )
			{
				/*   sprintf(localfile,"%s/data/%d/spj.c",OJ_HOME_PATH,p_id); */
				if ( access( localfile, 0 ) == 0 )
				{
					const char *cmd3 = "gcc -o %s/data/%d/spj %s/data/%d/spj.c";
					execute_cmd( cmd3, OJ_HOME_PATH, p_id, OJ_HOME_PATH, p_id );
				}
			}
			if ( strcmp( filename, "spj.cc" ) == 0 )
			{
				/*     sprintf(localfile,"%s/data/%d/spj.cc",OJ_HOME_PATH,p_id); */
				if ( access( localfile, 0 ) == 0 )
				{
					const char *cmd4 =
						"g++ -o %s/data/%d/spj %s/data/%d/spj.cc";
					execute_cmd( cmd4, OJ_HOME_PATH, p_id, OJ_HOME_PATH, p_id );
				}
			}
		}
	}
...
int main( int argc, char **argv )
{
...
#ifdef _mysql_h
	if ( !OJ_HTTP_JUDGE && !init_mysql_conn() )
	{
		// 当数据库连接失败时退出程序
	...
	sprintf( work_dir, "%s/run/%s/", OJ_HOME_PATH, argv[2] );
	...
	if ( lang >= 3 && lang != 10 && lang != 13 && lang != 14 && lang != 17 )
	{
		...
		/* the limit for java */
		time_lmt	= time_lmt + OJ_JAVA_TIME_BONUS;
		mem_lmt		= mem_lmt + OJ_JAVA_MEMORY_BONUS;
		/* copy java.policy */
		if ( lang == 3 )
		{
			execute_cmd( "/bin/cp %s/etc/java0.policy %s/java.policy", OJ_HOME_PATH, work_dir );
	...
	sprintf( fullpath, "%s/data/%d", OJ_HOME_PATH, p_id ); /* the fullpath of data dir */

	/* open DIRs */
	DIR *dp;
	dirent *dirp;
	/* using http to get remote test data files */
	if ( p_id > 0 && OJ_HTTP_JUDGE && http_download ) {
	...
	for (; (OJ_JUDGE_MODE || ACflg == OJ_AC || ACflg == OJ_PE) && (dirp = readdir( dp ) ) != NULL; )
	{
		namelen = isInFile( dirp->d_name ); /* check if the file is *.in or not */
		if ( namelen == 0 )
			continue;

		if ( OJ_HTTP_JUDGE && http_download && (!DATA_LIST_has( dirp->d_name ) ) )
			continue;
		...
		if ( pidApp == 0 )
		{
			...
			watch_solution( pidApp, infile, ACflg, isspj, userfile, outfile,
					solution_id, lang, topmemory, mem_lmt, usedtime, time_lmt,
					p_id, PEflg, work_dir );
			printf( "%s: mem=%d time=%d\n", infile + strlen( OJ_HOME_PATH ) + 5, topmemory, usedtime );
			total_time += usedtime;
			printf( "time:%d/%d\n", usedtime, total_time );
	...
	if ( OJ_SIM_ENABLE && ACflg == OJ_AC && (!OJ_JUDGE_MODE || finalACflg == OJ_AC) )
	{
	...
	if ( (OJ_JUDGE_MODE && finalACflg == OJ_RE) || ACflg == OJ_RE )
	{
		if ( DEBUG ) {
			printf( "add RE info of %d..... \n", solution_id );
		}
	...
	if ( DEBUG ) {
		write_log( "result=%d", OJ_JUDGE_MODE ? finalACflg : ACflg );
	}
